import {
  isStickmanEventV1,
  type StickmanPayloadV1
} from "./types.js";


export interface StickmanWebSocketOptions {
  url: string;
  topic: string;
  clientId?: string;
  reconnectInitialMs?: number;
  reconnectMaximumMs?: number;
  onPose: (payload: StickmanPayloadV1) => void;
...
export class StickmanWebSocketClient {
  private readonly options: Required<Pick<
    StickmanWebSocketOptions,
  ...
  private latest: StickmanPayloadV1 | null = null;

  constructor(options: StickmanWebSocketOptions) {
    this.options = {
      clientId: "mixamo-avatar-ui",
  ...
  get latestPose(): StickmanPayloadV1 | null {
    return this.latest;
  }
  ...
  private connect(): void {
    ...
    socket.addEventListener("open", () => {
      ...
      socket.send(JSON.stringify({
        type: "hello",
        message_id: messageId("frontend-hello"),
        client_type: "browser",
    ...
    socket.addEventListener("message", (event) => {
      ...
      if (!isStickmanEventV1(message, this.options.topic)) return;
      this.latest = message.payload;
      this.options.onPose(message.payload);
    });
